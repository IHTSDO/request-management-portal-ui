...
Key characteristics:
* Modern Angular application with **Server-Side Rendering (SSR)** for SEO and first-paint performance.
* **Responsive** UI powered by **Angular Material** and **TailwindCSS**.
...
## 3. Prerequisites
* **Node.js 18+** (aligned with `@types/node` in `package.json`).
* **npm 9+** or **pnpm 8+** (the repository is npm-centric but compatible with pnpm).
...
## 7. Styling & Design
* **TailwindCSS 3** provides utility-first classes. The configuration lives in `tailwind.config.js`.
* **Angular Material** components ensure accessible UI primitives.
...
* **Accessibility (a11y)**: Every interactive element must be keyboard-navigable and have an ARIA label.
* **Linting & Formatting**: (TBC) Adopt **@angular-eslint** and **Prettier** – see future roadmap.
